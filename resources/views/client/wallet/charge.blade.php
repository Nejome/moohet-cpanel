                ...
                <div class="card shadow">
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col text-left ml-xl-6">
                                <a href="#" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#charge_form_modal">شحن رصيد</a>
                            </div>
                            @include('client.wallet.charge_form_modal')
                        </div>
                        <!-- Public Errors -->
                        @if($errors->any())
                        ...
                        @if(session()->has('some_error'))
                            <div class="alert alert-danger mt-3">{{session()->get('some_error')}}</div>
                        @endif

                        @if(session()->has('after_error'))
                            <div class="alert alert-danger mt-3">{{session()->get('after_error')}}</div>
                        @endif

                        @if(session()->has('will_review_msg'))
                            <div class="alert alert-info mt-3">{{session()->get('will_review_msg')}}</div>
                        @endif

                        @if(session()->has('transaction_complete'))
                            <div class="alert alert-info mt-3">{{session()->get('transaction_complete')}}</div>
                        @endif
